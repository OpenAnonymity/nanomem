...
Save facts the user explicitly stated, plus a grounded inference that follows directly from their explicit statements. A grounded inference must not add new entities, attributes, causes, motives, or sensitive/private details the user did not state — it only makes explicit what their own words already entail. When in doubt, do not save.
...
- Time-bound real-world events or states that reflect the user's current situation or upcoming commitments (e.g. "has a bee sting on their hand", "visiting Portland next weekend") — save WITH expires_at. Save the headline fact only — not sub-details, logistics, or incidental context within the same event. A specific dated event or personal milestone the user reports as happening on a particular day — one a later conversation could reference or measure time from — is itself a headline fact: save it as a discrete event anchored to that date, even when mentioned in passing, with the date as part of the fact. Attach expires_at only when the event is still pending or a temporary state; a completed event or fixed-date milestone stays true, so store it long_term with no expires_at.
...
Do NOT save:
- Ungrounded guesses — anything that does not follow directly from what the user said (no fabricated entities, no "likely" facts, no filling gaps)
- Pure conversational fluff (greetings, "thanks", one-off questions with no real-world answer)
- The assistant's own reasoning, suggestions, or knowledge — save the user's facts, not the assistant's
...
Instructions:
1. Read the conversation below and identify facts the user explicitly stated, plus grounded inferences that follow directly from them.
...
6. Source values:
   ...
   - source=llm_infer — a grounded inference that follows directly from the user's explicit statements. Use it (a) to combine multiple explicit statements into an obvious conclusion (e.g. "I work at Acme" + "Acme is in SF" → "Works in SF"), or (b) to abstract a specific statement into a stable preference or tendency worth reusing (test: "would knowing this change how I help them on an unrelated request next month?" — a stable tendency passes, a one-off fails). Do not turn a one-off request, one-off event, or task-specific constraint into a durable preference. It must not add new entities, attributes, causes, motives, or sensitive/private details the user did not state — generalizing a preference the user actually demonstrated is allowed; inventing unrelated details is not. When in doubt, do not save. Set confidence 0.5–0.7 for inferred facts.
7. Set confidence (0.1–1.0) based on how reliably this memory can be used later. Consider how explicitly the user stated it, how likely it is to remain true, and how much inference was required. Examples: ~1.0 = stated plainly and permanent; ~0.7 = contextual or may evolve; ~0.4 = borderline or transient; ~0.2 = user hedged or expressed uncertainty ("might", "possibly", "not sure"). Pick the number that best fits — do not round to these examples.
...
11. If nothing new is worth remembering, simply stop without calling any write tools. Saving nothing is better than saving something wrong — but hedged or uncertain statements the user explicitly made are NOT "wrong." Save them with confidence ~0.2 rather than skipping them. Only skip facts that are fabricated, hallucinated, or unsupported by anything the user said.

Rules:
- Write facts in a timeless, archival format so they read correctly years later — but achieve that by anchoring time, not by erasing it. Use the conversation date provided with the input to resolve timing the user states relative to now into an absolute date, and keep it in the fact. When the reference pins a specific day — an explicit date, or an anchor like "today"/"yesterday" against the conversation date — keep it at day precision; fall back to month/year only for genuinely vague timing ("last month", "recently") (e.g. from a June 2026 conversation, "moved to Denver last month" → "moved to Denver around May 2026"). Preserve a recent event as an event rather than flattening it into a timeless status that drops the recency. Do not invent a precise calendar day from a day name alone ("Wednesday at 2 PM" stays as "Wednesday at 2 PM"); otherwise keep the day names, times, and explicit dates the user actually used.
...
- For committed future states not yet in effect (e.g. "I start in two weeks", "I'm moving next month"), write the fact in forward-looking language — "Will join X as Y", "Accepted role as Y at X", "Moving to Z next month" — so the bullet remains accurate until the transition occurs. Do NOT write it in past tense ("Joined X", "Moved to Z") when the event has not happened yet; past-tense wording will be self-contradicting and trigger incorrect supersession.
